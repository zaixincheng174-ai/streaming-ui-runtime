...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5u_b2c_multistream_agent_trace_stress.html");
const manualInputPath = "bench/p5/results/p5u_b2c_multistream_agent_trace_results.manual-input.json";
const collectorCommand =
  "node scripts/p5/collect_p5u_manual_b2c_multistream_agent_trace_results.mjs " + manualInputPath;

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-U B2u target");

  const matrix = JSON.parse(fs.readFileSync(matrixPath, "utf8"));
  const scenarios = Array.isArray(matrix.scenarios) ? matrix.scenarios : [];

  console.log("P5-U B2u Multistream Agent Trace Manual URL Generator");
  console.log("target=bench/p5/targets/p5u_b2c_multistream_agent_trace_stress.html");
  console.log(`scenario_count=${scenarios.length}`);
  console.log("");
  console.log("Manual steps:");
  console.log("1. open URL in Chrome");
  console.log("2. wait for #p5u-b2c-summary-json to show scenario_id");
  console.log("3. click concurrent_input_during_multistream_proxy");
  console.log("4. copy #p5u-b2c-summary-json");
