...
const resultPath = path.join(repoRoot, "bench/p5/results/p5o_r0c_commit_window_input_results.json");
const targetPath = "bench/p5/targets/p5o_r0c_commit_window_input_stress.html";
const runnerLabel = "p5o_r0c_commit_window_input_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "r0c_worker_bounded_projection_compact_context_commit_window_input";
...
const boundary = {
  baseline: "R0c commit-window worker/bounded-projection compact-context path only",
  does_not_measure: [
    "B2o",
  ...
  notes:
    "P5-O measures R0c commit-window internal scheduling-delay proxy metrics only. It preserves the P5-D matrix, compact active-context semantics, full logical transcript, tail mutation, append stream, and bounded rendered window."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; R0c commit-window worker/bounded-projection compact-context proxy metrics only."
  }));

  const result = {
    schema_version: "p5o.r0c-commit-window-input-results.v0",
    ...
    summary: {
      ...
      max_commit_window_worker_send_processing_ms: maxMetric(resultRows, "commit_window_worker_send_processing_ms"),
      max_commit_window_worker_roundtrip_minus_processing_ms: maxMetric(
        resultRows,
        "commit_window_worker_roundtrip_minus_processing_ms"
      ),
  ...
  console.log("P5-O Manual R0c Commit-Window Input Result Collector");
  ...
  console.log(
    `max_commit_window_worker_send_processing_ms=${formatMetric(
      result.summary.max_commit_window_worker_send_processing_ms
    )}`
  );
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5o_manual_r0c_commit_window_results.mjs bench/p5/results/p5o_r0c_commit_window_input_results.manual-input.json"
  );
}
...
function validateManualRows(rows, expectedScenarios) {
  ...
  for (const row of rows) {
    ...
    if (row.commit_window_worker_roundtrip_minus_processing_ms < 0) {
      throw new Error(`${row.scenario_id} commit_window_worker_roundtrip_minus_processing_ms must be >= 0`);
    }
  }
}
...
main().catch((error) => {
  console.error(`P5-O R0c manual collector failed: ${error.message}`);
  process.exit(1);
});
