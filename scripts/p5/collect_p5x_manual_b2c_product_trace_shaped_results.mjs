...
const resultPath = path.join(repoRoot, "bench/p5/results/p5x_b2c_product_trace_shaped_results.json");
const targetPath = "bench/p5/targets/p5x_b2c_product_trace_shaped_stress.html";
const runnerLabel = "p5x_b2c_product_trace_shaped_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "b2c_virtualized_dom_compact_context_product_trace_shaped";
...
const boundary = {
  baseline: "B2c product-trace-shaped compact dynamic-context baseline only",
  does_not_measure: [
    "R0x",
    "real product trace",
  ...
  notes:
    "P5-X measures B2x product-trace-shaped proxy metrics only. It is a fictional synthetic trace, not a real product trace. It preserves the P5-D matrix, compact scan semantics, rolling-tail dynamic context, full logical transcript, tail mutation, append stream, and bounded rendered window."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B2x product-trace-shaped synthetic proxy metrics only; not real product trace."
  }));

  const result = {
    schema_version: "p5x.b2c-product-trace-shaped-results.v0",
  ...
  console.log("P5-X Manual B2x Product Trace Shaped Result Collector");
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5x_manual_b2c_product_trace_shaped_results.mjs bench/p5/results/p5x_b2c_product_trace_shaped_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-X B2x manual collector failed: ${error.message}`);
  process.exit(1);
});
