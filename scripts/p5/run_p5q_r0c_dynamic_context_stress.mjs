...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5q_r0c_dynamic_context_stress.html");
const manualInputPath = "bench/p5/results/p5q_r0c_dynamic_context_results.manual-input.json";
const collectorCommand =
  "node scripts/p5/collect_p5q_manual_r0c_dynamic_context_results.mjs " + manualInputPath;

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-Q R0q target");

  const matrix = JSON.parse(fs.readFileSync(matrixPath, "utf8"));
  const scenarios = Array.isArray(matrix.scenarios) ? matrix.scenarios : [];

  console.log("P5-Q R0q Dynamic Active-Context Manual URL Generator");
  console.log("target=bench/p5/targets/p5q_r0c_dynamic_context_stress.html");
  console.log(`scenario_count=${scenarios.length}`);
  console.log("");
  console.log("Manual steps:");
  console.log("1. open URL in Chrome");
  console.log("2. wait for #p5q-r0c-summary-json to show scenario_id");
  console.log("3. click dynamic_context_send_proxy");
  console.log("4. copy #p5q-r0c-summary-json");
