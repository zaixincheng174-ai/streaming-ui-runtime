...
const resultPath = path.join(repoRoot, "bench/p5/results/p5u_b2c_multistream_agent_trace_results.json");
const targetPath = "bench/p5/targets/p5u_b2c_multistream_agent_trace_stress.html";
const runnerLabel = "p5u_b2c_multistream_agent_trace_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "b2c_virtualized_dom_compact_context_multistream_agent_trace";
...
const boundary = {
  baseline: "B2c multistream agent-trace compact dynamic-context baseline only",
  does_not_measure: [
    "R0u",
  ...
  notes:
    "P5-U measures B2u multistream agent-trace proxy metrics only. It preserves the P5-D matrix, compact scan semantics, rolling-tail dynamic context, full logical transcript, tail mutation, append stream, and bounded rendered window."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B2u multistream agent-trace proxy metrics only."
  }));

  const result = {
    schema_version: "p5u.b2c-multistream-agent-trace-results.v0",
  ...
  console.log("P5-U Manual B2u Multistream Agent Trace Result Collector");
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5u_manual_b2c_multistream_agent_trace_results.mjs bench/p5/results/p5u_b2c_multistream_agent_trace_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-U B2u manual collector failed: ${error.message}`);
  process.exit(1);
});
