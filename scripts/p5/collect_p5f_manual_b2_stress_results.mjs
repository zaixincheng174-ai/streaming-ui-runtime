...
const resultPath = path.join(repoRoot, "bench/p5/results/p5f_b2_virtualized_dom_stress_results.json");
const targetPath = "bench/p5/targets/p5f_b2_virtualized_dom_stress.html";
const runnerLabel = "p5f_b2_virtualized_dom_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "b2_virtualized_dom";
...
const requiredMetricFields = [...requiredCommonMetricFields, ...requiredB2MetricFields];
...
const boundary = {
  baseline: "B2 virtualized DOM stress baseline only",
  does_not_measure: [
    "B0 new measurement",
    "B1 new measurement",
    "B3",
  ...
  notes:
    "P5-F measures B2 virtualized DOM stress proxy metrics only. It preserves full logical transcript and active-context traversal while bounding mounted DOM. It does not compare against runtime path and does not authorize P4."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B2 virtualized DOM stress proxy metrics only."
  }));

  const result = {
    schema_version: "p5f.b2-virtualized-dom-stress-results.v0",
    ...
    summary: {
      ...
      max_virtual_window_render_ms: maxMetric(resultRows, "virtual_window_render_ms"),
  ...
  console.log("P5-F Manual B2 Virtualized DOM Stress Result Collector");
  ...
  console.log(`max_rendered_dom_node_count=${formatMetric(result.summary.max_rendered_dom_node_count)}`);
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5f_manual_b2_stress_results.mjs bench/p5/results/p5f_b2_virtualized_dom_stress_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-F manual collector failed: ${error.message}`);
  process.exit(1);
});
