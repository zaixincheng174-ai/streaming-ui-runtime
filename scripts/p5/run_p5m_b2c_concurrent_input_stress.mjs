...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5m_b2c_concurrent_input_stress.html");
const manualInputPath = "bench/p5/results/p5m_b2c_concurrent_input_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5m_b2c_concurrent_input_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-M B2m target");
...
function printManualWorkflow(urls) {
  console.log("P5-M B2c Concurrent Input Manual URL Generator");
  ...
  console.log("2. Wait for #p5m-b2c-summary-json to show the scenario_id.");
  console.log("3. Click concurrent_input_during_send_proxy.");
  console.log("4. Copy the JSON object from #p5m-b2c-summary-json.");
  ...
  console.log("- B2c concurrent-input compact-context baseline only");
  ...
  console.log(`node scripts/p5/collect_p5m_manual_b2c_concurrent_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5m.b2c-concurrent-input-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5m-b2c-summary-json"
    }))
  };
}
