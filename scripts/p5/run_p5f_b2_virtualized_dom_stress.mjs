...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5f_b2_virtualized_dom_stress.html");
const manualInputPath = "bench/p5/results/p5f_b2_virtualized_dom_stress_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5f_b2_virtualized_dom_stress_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-F target");
...
function printManualWorkflow(urls) {
  console.log("P5-F B2 Virtualized DOM Stress Manual URL Generator");
  ...
  console.log("2. Wait for #p5f-summary-json to show the scenario_id.");
  console.log("3. Click typing_proxy.");
  console.log("4. Click send_click_proxy.");
  console.log("5. Click scroll_jump_return.");
  console.log("6. Copy the JSON object from #p5f-summary-json.");
  ...
  console.log("- B2 virtualized DOM stress baseline only");
  console.log("- reuses the P5-D stress matrix");
  console.log("- full logical transcript and active-context traversal remain in JS");
  ...
  console.log("- no B0/B1 new measurement, B3, or R0 comparison");
  console.log();
  console.log("After all rows are pasted, run:");
  console.log(`node scripts/p5/collect_p5f_manual_b2_stress_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5f.b2-virtualized-dom-stress-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5f-summary-json"
    }))
  };
}
