...
const resultPath = path.join(repoRoot, "bench/p5/results/p5s_b2c_concurrent_dynamic_context_results.json");
const targetPath = "bench/p5/targets/p5s_b2c_concurrent_dynamic_context_stress.html";
const runnerLabel = "p5s_b2c_concurrent_dynamic_context_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "b2c_virtualized_dom_compact_context_concurrent_dynamic_context";
...
const boundary = {
  baseline: "B2c concurrent-input dynamic active-context baseline only",
  does_not_measure: [
    "R0s",
  ...
  notes:
    "P5-S measures B2s concurrent-input during dynamic active-context update proxy metrics only. It preserves the P5-D matrix, compact scan semantics, rolling-tail dynamic context, full logical transcript, tail mutation, append stream, and bounded rendered window."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B2s concurrent dynamic active-context proxy metrics only."
  }));

  const result = {
    schema_version: "p5s.b2c-concurrent-dynamic-context-results.v0",
  ...
  console.log("P5-S Manual B2s Concurrent Dynamic Context Result Collector");
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5s_manual_b2c_concurrent_dynamic_context_results.mjs bench/p5/results/p5s_b2c_concurrent_dynamic_context_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-S B2s manual collector failed: ${error.message}`);
  process.exit(1);
});
