...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5i_b2_compact_context_stress.html");
const manualInputPath = "bench/p5/results/p5i_b2_compact_context_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5i_b2_compact_context_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-I B2c target");
...
function printManualWorkflow(urls) {
  console.log("P5-I B2c Compact Active-Context Manual URL Generator");
  ...
  console.log("2. Wait for #p5i-b2-summary-json to show the scenario_id.");
  console.log("3. Click typing_proxy.");
  console.log("4. Click send_click_proxy.");
  console.log("5. Click scroll_jump_return.");
  console.log("6. Copy the JSON object from #p5i-b2-summary-json.");
  ...
  console.log("- B2c virtualized DOM compact-context stress baseline only");
  ...
  console.log("- compact active-context index is algorithmic and paired with R0c");
  ...
  console.log("- no B0/B1/B2 original/R0/R0c/B3 measurement");
  console.log();
  console.log("After all rows are pasted, run:");
  console.log(`node scripts/p5/collect_p5i_manual_b2_compact_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5i.b2-compact-context-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5i-b2-summary-json"
    }))
  };
}
