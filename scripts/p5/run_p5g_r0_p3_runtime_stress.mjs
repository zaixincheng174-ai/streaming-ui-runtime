...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5g_r0_p3_runtime_stress.html");
const manualInputPath = "bench/p5/results/p5g_r0_p3_runtime_stress_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5g_r0_p3_runtime_stress_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-G target");
...
function printManualWorkflow(urls) {
  console.log("P5-G R0 P3-Derived Runtime Stress Manual URL Generator");
  ...
  console.log("2. Wait for #p5g-summary-json to show the scenario_id and initial render.");
  console.log("3. Click typing_proxy.");
  console.log("4. Click send_click_proxy.");
  console.log("5. Click scroll_jump_return.");
  console.log("6. Copy the JSON object from #p5g-summary-json.");
  ...
  console.log("- R0 P3-derived worker/bounded-projection runtime path only");
  ...
  console.log("- no B0/B1/B2 new measurement or B3 comparison");
  console.log();
  console.log("After all rows are pasted, run:");
  console.log(`node scripts/p5/collect_p5g_manual_r0_stress_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5g.r0-p3-runtime-stress-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5g-summary-json"
    }))
  };
}
