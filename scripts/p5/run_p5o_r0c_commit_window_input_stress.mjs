...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5o_r0c_commit_window_input_stress.html");
const manualInputPath = "bench/p5/results/p5o_r0c_commit_window_input_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5o_r0c_commit_window_input_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-O R0o target");
...
function printManualWorkflow(urls) {
  console.log("P5-O R0c Commit-Window Input Manual URL Generator");
  ...
  console.log("2. Wait for #p5o-r0c-summary-json to show the scenario_id.");
  console.log("3. Click commit_window_input_during_send_proxy.");
  console.log("4. Copy the JSON object from #p5o-r0c-summary-json.");
  ...
  console.log("- R0c commit-window worker/bounded-projection compact-context path only");
  ...
  console.log(`node scripts/p5/collect_p5o_manual_r0c_commit_window_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5o.r0c-commit-window-input-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5o-r0c-summary-json"
    }))
  };
}
