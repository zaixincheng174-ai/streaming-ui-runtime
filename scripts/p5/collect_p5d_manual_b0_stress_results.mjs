...
const resultPath = path.join(repoRoot, "bench/p5/results/p5d_b0_stress_calibration_results.json");
const targetPath = "bench/p5/targets/p5d_b0_stress_calibration.html";
const runnerLabel = "p5d_b0_stress_manual_matrix";
...
const boundary = {
  baseline: "B0 naive DOM stress calibration only",
  does_not_measure: [
    "B1",
    "B2",
    "B3",
  ...
  notes:
    "P5-D calibrates B0 stress pressure only. It does not compare against alternative baselines or runtime path."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B0 stress calibration proxy metrics only."
  }));

  const result = {
    schema_version: "p5d.b0-stress-calibration-results.v0",
  ...
  console.log("P5-D Manual B0 Stress Result Collector");
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5d_manual_b0_stress_results.mjs bench/p5/results/p5d_b0_stress_calibration_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-D manual collector failed: ${error.message}`);
  process.exit(1);
});
