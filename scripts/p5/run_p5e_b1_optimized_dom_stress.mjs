...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5e_b1_optimized_dom_stress.html");
const manualInputPath = "bench/p5/results/p5e_b1_optimized_dom_stress_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5e_b1_optimized_dom_stress_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-E target");
...
function printManualWorkflow(urls) {
  console.log("P5-E B1 Optimized/Batched DOM Stress Manual URL Generator");
  ...
  console.log("2. Wait for #p5e-summary-json to show the scenario_id.");
  console.log("3. Click typing_proxy.");
  console.log("4. Click send_click_proxy.");
  console.log("5. Click scroll_jump_return.");
  console.log("6. Copy the JSON object from #p5e-summary-json.");
  ...
  console.log("- B1 optimized/batched DOM stress baseline only");
  ...
  console.log("- no B0 new measurement, B2, B3, or R0 comparison");
  console.log();
  console.log("After all rows are pasted, run:");
  console.log(`node scripts/p5/collect_p5e_manual_b1_stress_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5e.b1-optimized-dom-stress-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5e-summary-json"
    }))
  };
}
