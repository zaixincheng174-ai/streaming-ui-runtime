...
const resultPath = path.join(repoRoot, "bench/p5/results/p5q_b2c_dynamic_context_results.json");
const targetPath = "bench/p5/targets/p5q_b2c_dynamic_context_stress.html";
const runnerLabel = "p5q_b2c_dynamic_context_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "b2c_virtualized_dom_compact_context_dynamic_active_context";
...
const boundary = {
  baseline: "B2c dynamic active-context compact baseline only",
  does_not_measure: [
    "R0q",
  ...
  notes:
    "P5-Q measures B2c dynamic active-context compact-context proxy metrics only. It preserves the P5-D matrix, full logical transcript, compact scan entry visits, tail mutation, append stream, and bounded rendered window. It does not authorize P4."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B2q dynamic active-context compact-context proxy metrics only."
  }));

  const result = {
    schema_version: "p5q.b2c-dynamic-context-results.v0",
  ...
  console.log("P5-Q Manual B2q Dynamic Context Result Collector");
  ...
  console.log(`max_dynamic_active_context_update_ms=${formatMetric(result.summary.max_dynamic_active_context_update_ms)}`);
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5q_manual_b2c_dynamic_context_results.mjs bench/p5/results/p5q_b2c_dynamic_context_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-Q B2q manual collector failed: ${error.message}`);
  process.exit(1);
});
