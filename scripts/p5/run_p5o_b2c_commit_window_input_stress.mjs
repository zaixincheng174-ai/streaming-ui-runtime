...
const targetPath = path.join(repoRoot, "bench/p5/targets/p5o_b2c_commit_window_input_stress.html");
const manualInputPath = "bench/p5/results/p5o_b2c_commit_window_input_results.manual-input.json";
const finalResultPath = "bench/p5/results/p5o_b2c_commit_window_input_results.json";

function main() {
  assertFile(matrixPath, "P5-D matrix");
  assertFile(targetPath, "P5-O B2o target");
...
function printManualWorkflow(urls) {
  console.log("P5-O B2c Commit-Window Input Manual URL Generator");
  ...
  console.log("2. Wait for #p5o-b2c-summary-json to show the scenario_id.");
  console.log("3. Click commit_window_input_during_send_proxy.");
  console.log("4. Copy the JSON object from #p5o-b2c-summary-json.");
  ...
  console.log("- B2c commit-window compact-context baseline only");
  ...
  console.log(`node scripts/p5/collect_p5o_manual_b2c_commit_window_results.mjs ${manualInputPath}`);
  console.log();
...
function manualInputTemplate(urls) {
  return {
    schema_version: "p5o.b2c-commit-window-input-manual-input.v0",
    collection_mode: "manual_user_chrome",
    rows: urls.map((entry) => ({
      scenario_id: entry.scenario_id,
      paste_from: "#p5o-b2c-summary-json"
    }))
  };
}
