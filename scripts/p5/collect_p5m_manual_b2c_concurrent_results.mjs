...
const resultPath = path.join(repoRoot, "bench/p5/results/p5m_b2c_concurrent_input_results.json");
const targetPath = "bench/p5/targets/p5m_b2c_concurrent_input_stress.html";
const runnerLabel = "p5m_b2c_concurrent_input_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "b2c_virtualized_dom_compact_context_concurrent_input";
...
const boundary = {
  baseline: "B2c concurrent-input compact-context baseline only",
  does_not_measure: [
    "R0m",
  ...
  notes:
    "P5-M measures B2c concurrent-input internal scheduling-delay proxy metrics only. It preserves the P5-D matrix, compact active-context semantics, full logical transcript, tail mutation, append stream, and bounded rendered window."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; B2c concurrent-input compact-context proxy metrics only."
  }));

  const result = {
    schema_version: "p5m.b2c-concurrent-input-results.v0",
  ...
  console.log("P5-M Manual B2c Concurrent Input Result Collector");
  ...
  console.log(`max_concurrent_send_work_ms=${formatMetric(result.summary.max_concurrent_send_work_ms)}`);
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5m_manual_b2c_concurrent_results.mjs bench/p5/results/p5m_b2c_concurrent_input_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-M B2c manual collector failed: ${error.message}`);
  process.exit(1);
});
