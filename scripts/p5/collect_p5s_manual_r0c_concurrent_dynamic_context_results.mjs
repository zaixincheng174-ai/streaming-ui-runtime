...
const resultPath = path.join(repoRoot, "bench/p5/results/p5s_r0c_concurrent_dynamic_context_results.json");
const targetPath = "bench/p5/targets/p5s_r0c_concurrent_dynamic_context_stress.html";
const runnerLabel = "p5s_r0c_concurrent_dynamic_context_manual_matrix";
const browserMode = "user_chrome_manual";
const baselineVariant = "r0c_worker_bounded_projection_compact_context_concurrent_dynamic_context";
...
const commonMetricFields = [
  ...
  "concurrent_dynamic_context_send_proxy_ms",
  "dynamic_update_input_delay_ms",
  "max_dynamic_update_input_delay_ms",
  "dynamic_update_input_scheduled_at_ms",
  "dynamic_update_input_started_at_ms",
...
const boundary = {
  baseline: "R0c concurrent-input dynamic active-context worker/bounded-projection path only",
  does_not_measure: [
    "B2s",
  ...
  notes:
    "P5-S measures R0s concurrent-input during Worker-side dynamic active-context update proxy metrics only. It preserves the P5-D matrix, compact scan semantics, rolling-tail dynamic context, full logical transcript, tail mutation, append stream, and bounded rendered projection."
};
...
async function main() {
  ...
  const resultRows = rows.map((row) => ({
    ...
    notes: "Manual user Chrome run; R0s concurrent dynamic active-context proxy metrics only."
  }));

  const result = {
    schema_version: "p5s.r0c-concurrent-dynamic-context-results.v0",
    ...
    rows: resultRows,
    summary: {
      max_dynamic_update_input_delay_ms: maxMetric(resultRows, "dynamic_update_input_delay_ms"),
      max_concurrent_dynamic_context_send_proxy_ms: maxMetric(resultRows, "concurrent_dynamic_context_send_proxy_ms"),
      max_dynamic_update_typing_proxy_ms: maxMetric(resultRows, "dynamic_update_typing_proxy_ms"),
      max_dynamic_update_send_work_ms: maxMetric(resultRows, "dynamic_update_send_work_ms"),
      ...
      max_concurrent_worker_dynamic_context_processing_ms: maxMetric(
        resultRows,
        "concurrent_worker_dynamic_context_processing_ms"
      ),
  ...
  console.log("P5-S Manual R0s Concurrent Dynamic Context Result Collector");
  ...
  console.log(`max_dynamic_update_input_delay_ms=${formatMetric(result.summary.max_dynamic_update_input_delay_ms)}`);
...
function usage() {
  console.error("Usage:");
  console.error(
    "  node scripts/p5/collect_p5s_manual_r0c_concurrent_dynamic_context_results.mjs bench/p5/results/p5s_r0c_concurrent_dynamic_context_results.manual-input.json"
  );
}
...
main().catch((error) => {
  console.error(`P5-S R0s manual collector failed: ${error.message}`);
  process.exit(1);
});
